...
class TradeMatcher:
    ...
    def match_trades(
        ...
        for wallet_address, wallet_trade_list in wallet_trades.items():
            ...
            if min_trade_size_usd:
                wallet_trade_list = [
                    t for t in wallet_trade_list if t.get("amount_usd", 0) >= min_trade_size_usd
                ]
            ...
            # Calculate holdings
            current_holdings_usd = self._calculate_current_holdings(wallet_trade_list)
            ...
            matched_wallets[wallet_address] = {
                ...
                "total_buy_volume_usd": sum(
                    t.get("amount_usd", 0) for t in wallet_trade_list if t.get("side") == "buy"
                ),
                "total_sell_volume_usd": sum(
                    t.get("amount_usd", 0) for t in wallet_trade_list if t.get("side") == "sell"
                ),
            }
    ...
    def _match_buys_and_sells(
        ...
        """
        # Sort by timestamp
        sorted_trades = sorted(trades, key=lambda t: self._parse_timestamp(t.get("timestamp")))
    ...
    def _calculate_current_holdings(
        ...
        total_bought = sum(t.get("amount_usd", 0) for t in trades if t.get("side") == "buy")
        total_sold = sum(t.get("amount_usd", 0) for t in trades if t.get("side") == "sell")
