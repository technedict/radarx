...
class SmartWalletBacktester:
    ...
    def run_backtest(
        ...
        top_wallets = [
            w["wallet_address"]
            for w in smart_wallet_rankings[:self.config.top_k]
        ]
        ...
        while current_date <= end_date:
            ...
            for trade in day_trades:
                # Execute copy trade
                result = self._execute_copy_trade(
                    portfolio, trade, price_data, current_date
                )
            ...
            portfolio_value = self._calculate_portfolio_value(
                portfolio, price_data, current_date
            )
    ...
    def _execute_copy_trade(
        ...
        if side == "buy":
            ...
            position_value = min(
                portfolio["cash"] * self.config.position_size,
            ...
            if portfolio["cash"] >= cost:
                ...
                ) if current_quantity > 0 else effective_price
    ...
    def _calculate_metrics(
        ...
        profitable_trades = sum(
            ...
            if trade.get("side") == "sell" and trade.get("pnl", 0) > 0
        )
