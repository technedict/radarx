...
class TimingSignalDetector:
    ...
    def _find_next_peak(
        ...
        future_prices = [
            p for p in price_timeline
            if self._parse_timestamp(p.get("timestamp")) > from_time
        ]
        ...
        for i in range(1, len(future_prices) - 1):
            if (future_prices[i].get("price", 0) > future_prices[i - 1].get("price", 0) and
                future_prices[i].get("price", 0) > future_prices[i + 1].get("price", 0)):
...
class GraphAnalyzer:
    ...
    def analyze(
        ...
        connected_smart = self._count_smart_connections(
            wallet_address, edges, smart_wallets
        )
...
class BehavioralAnalyzer:
    ...
    def _get_preferred_hours(
        ...
        hours = [
            self._parse_timestamp(t.get("timestamp")).hour
            for t in trades if "timestamp" in t
        ]
    ...
    def _detect_patterns(
        ...
        avg_duration = np.mean([
            t.get("holding_duration_hours", 0) for t in trades
        ])
