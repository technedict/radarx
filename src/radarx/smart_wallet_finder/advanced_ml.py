...
class GrangerCausalityAnalyzer:
    ...
    def analyze_wallet_price_causality(
        ...
        try:
            ...
            has_causality = bool(min_p_value < self.significance_level)

            # Calculate lead score: normalized by p-value strength
            lead_score = float(max(0.0, 1.0 - min_p_value)) if has_causality else 0.0

            return {
                "has_causality": has_causality,
                "optimal_lag": int(optimal_lag),
                "p_values": {int(k): float(v) for k, v in p_values.items()},
...
class CounterfactualAnalyzer:
    ...
    def estimate_wallet_impact(
        ...
        price_influence = self._estimate_price_influence(wallet_trades, price_timeline)
        ...
        impact_score = min(
            1.0,
            (0.4 * volume_contribution + 0.4 * price_influence + 0.2 * abs(counterfactual_returns)),
        )
