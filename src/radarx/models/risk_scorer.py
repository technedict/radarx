...
class RiskScorer:
    ...
    def train(
        self, X_features: np.ndarray, y_labels: Dict[str, np.ndarray], feature_names: List[str]
    ):
        """
    ...
    def score(self, X_features: np.ndarray) -> Dict[str, float]:
        ...
        composite = sum(
            scores[component] * self.component_weights.get(component, 0.2) for component in scores
        )
    ...
    def get_risk_flags(
        self, scores: Dict[str, float], features: Optional[Dict[str, float]] = None
    ) -> List[str]:
        """
