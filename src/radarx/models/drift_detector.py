...
class DriftDetector:
    ...
    def page_hinkley_test(
        self, values: List[float], delta: float = 0.005, lambda_: float = 50
    ) -> bool:
        """
    ...
    def reset_reference(self):
        ...
        self.reference_features = deque(self.current_features, maxlen=self.window_size)
        self.reference_predictions = deque(self.current_predictions, maxlen=self.window_size)
        self.reference_performance = deque(self.current_performance, maxlen=self.window_size)
    ...
    def get_drift_summary(self) -> Dict:
        ...
        most_common_type = (
            max(set(drift_types), key=drift_types.count) if drift_types else DriftType.NONE
        )
