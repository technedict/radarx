...
try:
    from river import ensemble, forest, metrics
...
class OnlineLearner:
    ...
    def partial_fit(self, X: np.ndarray, y: dict, sample_weights: Optional[np.ndarray] = None):
        """
        ...
        if sample_weights is None:
            n_samples = X.shape[0]
            decay_rate = 0.99
            sample_weights = np.array([decay_rate ** (n_samples - i - 1) for i in range(n_samples)])
    ...
    def should_retrain(self, current_performance: float) -> bool:
        ...
        if self.performance_history:
            ...
            perf_values = [
                h["performance"] for h in self.performance_history[-10:]
            ]  # Last 10 measurements
            baseline = np.mean(perf_values)
            if current_performance < baseline * (1 - self.retrain_threshold):
                logger.warning(
                    f"Performance drop detected: {current_performance:.3f} vs {baseline:.3f}"
                )
                return True
...
class IncrementalEnsemble:
    ...
    def __init__(self):
        ...
        # Metrics
        self.metrics = {"accuracy": metrics.Accuracy(), "f1": metrics.F1(), "auc": metrics.ROCAUC()}
