...
class WalletRanker:
    ...
    def get_rankings(
        ...
        sorted_wallets = sorted(filtered, key=lambda x: x.get(metric, 0), reverse=True)
    ...
    def get_wallet_rank(
        self, wallet_address: str, metric: str = "win_rate", chain: Optional[str] = None
    ...
    def get_top_wallets(
        self, n: int = 10, metric: str = "pnl", chain: Optional[str] = None, min_trades: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        return self.get_rankings(metric=metric, chain=chain, limit=n, min_trades=min_trades)
