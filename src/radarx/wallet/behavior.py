...
class BehaviorDetector:
    ...
    def detect_patterns(
        ...
        if token_listings:
            ...
            if early_conf > 0:
                patterns.append({"pattern": "early_adopter", "confidence": early_conf})

        # Diamond hands
        diamond_conf = self._check_diamond_hands(trades)
        if diamond_conf > 0:
            patterns.append({"pattern": "diamond_hands", "confidence": diamond_conf})

        # Swing trader
        swing_conf = self._check_swing_trader(trades)
        if swing_conf > 0:
            patterns.append({"pattern": "swing_trader", "confidence": swing_conf})

        # Day trader
        day_conf = self._check_day_trader(trades)
        if day_conf > 0:
            patterns.append({"pattern": "day_trader", "confidence": day_conf})

        # Smart money
        smart_conf = self._check_smart_money(trades)
        if smart_conf > 0:
            patterns.append({"pattern": "smart_money", "confidence": smart_conf})

        # Wash trader
        wash_score = self.calculate_wash_trading_score(trades)
        if wash_score > 0.3:
            patterns.append({"pattern": "wash_trader", "confidence": wash_score})

        # Pump chaser
        pump_conf = self._check_pump_chaser(trades)
        if pump_conf > 0:
            patterns.append({"pattern": "pump_chaser", "confidence": pump_conf})

        # Profit taker
        profit_conf = self._check_profit_taker(trades)
        if profit_conf > 0:
            patterns.append({"pattern": "profit_taker", "confidence": profit_conf})

        # Loss cutter
        loss_conf = self._check_loss_cutter(trades)
        if loss_conf > 0:
            patterns.append({"pattern": "loss_cutter", "confidence": loss_conf})

        return patterns

    def _check_early_adopter(
        self, trades: List[Dict[str, Any]], token_listings: Dict[str, datetime]
    ) -> float:
    ...
    def _check_swing_trader(self, trades: List[Dict[str, Any]]) -> float:
        ...
        swing_trades = sum(
            1 for h in hold_times if self.SWING_TRADER_MIN_DAYS <= h <= self.SWING_TRADER_MAX_DAYS
        )
    ...
    def _check_smart_money(self, trades: List[Dict[str, Any]]) -> float:
        ...
        confidence = min(
            1.0, (win_rate - self.SMART_MONEY_WIN_RATE) / (1.0 - self.SMART_MONEY_WIN_RATE)
        )
        return confidence
    ...
    def _check_pump_chaser(self, trades: List[Dict[str, Any]]) -> float:
        ...
        for trade in trades:
            ...
            if (
                trade.get("action") == "buy" and trade.get("price_change_5m", 0) > 0.1
            ):  # >10% in 5min
                pump_buys += 1
    ...
    def _check_loss_cutter(self, trades: List[Dict[str, Any]]) -> float:
        ...
        for trade in loss_trades:
            if "buy_timestamp" in trade and "sell_timestamp" in trade:
                ...
                if (
                    hold_hours <= 24 and abs(trade["pnl"]) / trade.get("buy_amount", 1) < 0.2
                ):  # <20% loss
                    quick_cuts += 1
