...
from radarx.data import TwitterClient, RedditClient, DataNormalizer
...
class SocialFeatureExtractor:
    ...
    async def extract_mention_volume_features(
        ...
        features[f"total_mentions_{hours}h"] = (
            features[f"twitter_mentions_{hours}h"] +
            features[f"reddit_mentions_{hours}h"]
        )
        
        return features
    
    async def extract_mention_velocity_features(
        self,
        query: str
    ) -> Dict[str, float]:
    ...
    async def extract_engagement_features(
        self,
        query: str,
        hours: int = 24
    ) -> Dict[str, float]:
    ...
    def extract_sentiment_features(
        ...
        positive_words = {
            "bullish", "moon", "pump", "rocket", "gain", "profit",
            "up", "buy", "long", "hold", "diamond", "hands"
        }
        negative_words = {
            "bearish", "dump", "rug", "scam", "loss", "sell",
            "down", "short", "dead", "crash", "paper", "hands"
        }
