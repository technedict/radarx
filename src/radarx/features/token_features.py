...
from radarx.data import DexScreenerClient, DataNormalizer
...
class TokenFeatureExtractor:
    ...
    async def extract_market_features(
        ...
        # Price changes
        features["price_change_1h"] = DataNormalizer.clean_numeric(token_data.get("price_change_1h"), 0)
        features["price_change_6h"] = DataNormalizer.clean_numeric(token_data.get("price_change_6h"), 0)
        features["price_change_24h"] = DataNormalizer.clean_numeric(token_data.get("price_change_24h"), 0)
        ...
        features["txns_24h_sells"] = DataNormalizer.clean_numeric(token_data.get("txns_24h_sells"), 0)
    ...
    async def extract_smart_money_features(
        ...
        known_smart_wallets: Optional[List[str]] = None
    ) -> Dict[str, float]:
