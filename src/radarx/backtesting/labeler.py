...
class OutcomeLabeler:
        ...
        for horizon in self.horizons:
            ...
            for multiplier in self.multipliers:
                ...
                reached_count = sum(
                    ...
                    if label.labels.get(horizon, {}).get(multiplier, False)
                )
                censored_count = sum(
                    1 for label in labels
                    if label.censored.get(horizon, {}).get(multiplier, False)
                )
    ...
    def to_training_format(
        self,
        labels: List[OutcomeLabel],
        features: Dict[str, Dict]  # token_address -> feature dict
        ...
        for label in labels:
            ...
            for horizon in self.horizons:
                for multiplier in self.multipliers:
                    ...
                    label_dict[f'{key}_censored'] = (
                        label.censored.get(horizon, {}).get(multiplier, False)
                    )
                    
                    # Add time to event for survival analysis
                    label_dict[f'{key}_time'] = (
                        label.time_to_target.get(horizon, {}).get(multiplier)
                    )
