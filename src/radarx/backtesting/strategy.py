...
class StrategySimulator:
    ...
    def _threshold_strategy(
        ...
        if prob > threshold and len(self.positions) < self.max_positions:
            ...
            self._open_position(
                token_addr, data_point["timestamp"], data_point["price"], position_size
            )
    ...
    def _kelly_strategy(
        ...
        if f > 0:
            ...
            position_size = min(
                self.capital * f * kelly_fraction, self.capital * self.max_position_size
            )
    ...
    def _open_position(
        self, token_address: str, timestamp: datetime, price: float, position_size_usd: float
    ):
        """Open a new position."""
    ...
    def _close_position(self, token_address: str, timestamp: datetime, price: float):
        ...
        total_slippage = (
            position.cost_basis * self.slippage_factor + gross_proceeds * self.slippage_factor
        )
    ...
    def _get_price_at_time(
        self, token_address: str, timestamp: datetime, historical_data: List[Dict]
    ) -> Optional[float]:
