...
class ProbabilityWithConfidence(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "probability": 0.35,
                "confidence_interval": {"lower": 0.28, "upper": 0.42, "confidence_level": 0.95},
            }
        }
    )
...
class RiskScore(BaseModel):
    ...
    composite_score: float = Field(
        ..., ge=0, le=100, description="Overall risk score (0=lowest, 100=highest)"
    )
...
class Explanation(BaseModel):
    ...
    top_features: List[FeatureContribution] = Field(
        ..., max_length=5, description="Top 5 contributing features"
    )
