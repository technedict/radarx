...
class TestBehaviorDetector:
    ...
    def test_check_smart_money(self):
        ...
        is_smart = detector.is_smart_money(trades, threshold=0.4)  # Lower threshold for scaled confidence
...
class TestRelatedWalletFinder:
    ...
    def test_find_by_fund_flow(self):
        ...
        transfers = [
            {'from': '0x111', 'to': '0x222', 'amount': 1000, 'timestamp': datetime.utcnow().isoformat() + 'Z'},
            {'from': '0x222', 'to': '0x333', 'amount': 500, 'timestamp': datetime.utcnow().isoformat() + 'Z'},
        ]
