...
def test_score_token():
    ...
    response = client.get(
        "/score/token",
        params={"address": "0x1234567890abcdef1234567890abcdef12345678", "chain": "ethereum"},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_wallet_report():
    ...
    response = client.get(
        "/wallet/report",
        params={"address": "0xabcdef1234567890abcdef1234567890abcdef12", "period": "30d"},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_search_wallets():
    ...
    response = client.get(
        "/search/wallets", params={"min_win_rate": 0.6, "min_trades": 10, "limit": 50}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_alerts_subscribe():
    ...
    response = client.post(
        "/alerts/subscribe",
        params={"webhook_url": "https://example.com/webhook", "min_probability_10x": 0.5},
    )
    assert response.status_code == 200
    data = response.json()
