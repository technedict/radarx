...
def test_probability_with_confidence():
    ...
    prob = ProbabilityWithConfidence(
        probability=0.5, confidence_interval={"lower": 0.4, "upper": 0.6, "confidence_level": 0.95}
    )
...
def test_probability_validation():
    ...
    # Valid probability
    prob = ProbabilityWithConfidence(
        probability=0.5, confidence_interval={"lower": 0.4, "upper": 0.6}
    )
    assert prob.probability == 0.5

    # Invalid probability (> 1)
    with pytest.raises(ValueError):
        ProbabilityWithConfidence(probability=1.5, confidence_interval={"lower": 0.4, "upper": 0.6})
...
def test_win_rate():
    ...
    win_rate = WinRate(
        ...
        by_timeframe=WinRateByTimeframe(
            one_day=0.75, seven_day=0.70, thirty_day=0.68, all_time=0.68
        ),
...
def test_pnl_summary():
    ...
    pnl = PnLSummary(
        realized_pnl=RealizedPnL(
            total_usd=10000, average_per_trade_usd=100, best_trade_usd=5000, worst_trade_usd=-500
        ),
        total_volume=TotalVolume(
            buy_volume_usd=50000, sell_volume_usd=60000, total_volume_usd=110000
        ),
    )
