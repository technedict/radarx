...
class TestProbabilityPredictor:
    ...
    def test_dummy_predictions(self):
        ...
        predictions = predictor.predict_proba(
            X, horizons=["24h", "7d"], multipliers=["2x", "5x", "10x"]
        )
...
class TestRiskScorer:
    ...
    def test_risk_flags(self):
        ...
        features = {"dev_holding_pct": 0.30, "holder_gini": 0.85, "liquidity_usd": 5000}
        ...
        assert any(
            "dev" in flag.lower() or "hold" in flag.lower() or "liquidity" in flag.lower()
            for flag in flags
        )
...
def test_full_pipeline_integration(sample_features, sample_labels):
    ...
    predictor = ProbabilityPredictor()
    predictions = predictor.predict_proba(
        sample_features, horizons=["24h", "7d"], multipliers=["2x", "5x", "10x"]
    )
    ...
    for i in range(len(sample_features)):
        detector.update(sample_features[i], list(predictions.values())[0], is_reference=(i < 50))
